...
public class test extends BaseTest {
	...
	public Object[][] getData() {
		...
		String s[][] = utility.getDataFromCSV(workingdirectory + "\\products3.csv", colNameToLocation);
		String random[][] = null;
	...
	public void testCSVData(String Sku, String goldweight) throws InterruptedException {
		String s = Sku.substring(0, Sku.indexOf('-'));
		initilizeEtry();
		...
		Assert.assertTrue(e.isEnabled());// return true when the buy now button
											// enable
		...
		/**
		 * verification of all sku ids with all different sizes of each random
		 * products with diamond quality SI-GH
		 * 
		 */
		...
		for (WebElement j : e1) {
			...
			if (actualsku.toUpperCase().trim().equals(Sku.toUpperCase().trim())) {
				String actualGoldWeight = utility1.convertDecimal(productItms.getGoldWeight().split(" ")[0]);
		...
		/**
		 * verification of all sku ids with all different sizes of each random
		 * products with diamond quality VVS-EF
		 * 
		 */
		...
		driver.findElement(
				...
				.click();
		e1 = new ArrayList<WebElement>(driver.findElements(By.xpath("//select/option")));
		for (WebElement j : e1) {
			j.click();
			Thread.sleep(5000);
			// verification of sku of the product for SI-GH
	...
	public void createHtML() {
		htmlReport = "<!DOCTYPE html><html><head><p>Hi All,</p>"
				...
				+ "<table ><tr>" + "<th>Result</th>" + "<th>Sku</th>" + "<th>goldweight</th>" + "</tr>";
	}
